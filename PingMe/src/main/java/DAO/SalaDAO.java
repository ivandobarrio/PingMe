...
public class SalaDAO {
    ...
    public Sala obtenerPorNombre(String nombre) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            Query<Sala> q = session.createQuery(
                "FROM Sala WHERE nombre = :nombre",
                Sala.class
            );
    ...
    public List<Sala> obtenerSalasDeUsuario(Long userId) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            Query<Sala> q = session.createQuery(
                "SELECT s FROM Sala s JOIN s.integrantes u WHERE u.id = :uid",
                Sala.class
            );
    ...
    public void agregarUsuarioASala(Long salaId, Usuario usuario) {
        Transaction tx = null;
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            tx = session.beginTransaction();
            ...
            if (sala != null) {
                sala.getIntegrantes().add(usuario);
                session.update(sala);
            }
    ...
    public int eliminarSalasVacias() {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            Query<?> q = session.createQuery(
                "DELETE FROM Sala s WHERE size(s.integrantes) = 0"
            );
